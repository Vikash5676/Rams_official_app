...
function DispatchMaterialModal({ data, method }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const res = window.api.addDispatchMaterial(formData);
    if (res) {
  ...
  useEffect(() => {
    if (formData.qty > stockValue) {
      setValidation({
        message: `You cannot enter more quantity than stock`,
        validate: false,
      });
    } else {
      setValidation({ message: "", validate: true });
      setStockData({
        ...stockData,
        ["stock"]: parseInt(stockValue) - parseInt(formData.qty),
      });
    }
  ...
  return (
    ...
    >
      <Modal.Header>
        {method === "POST" ? <>Dispatch Material</> : <>Edit Job Details</>}
      </Modal.Header>
      <Modal.Content>
        ...
        >
          ...
          <Form.Field className="flex-[30%]">
            <label htmlFor="">Receiver company / Consumed by</label>
            <input
              type="text"
              name="person"
              onChange={handleChange}
